...
public class TexturedTriangle extends TexturedQuad
{
    ...
    //	System.out.println(String.format("glColor : %x ",color));
        ...
        for (int i = 0; i < nVertices; i++)
        {
        	...
        	if (invertNormal)
        	{
        		//par1Tessellator.setNormal(-(float)vec3d2[i].x, -(float)vec3d2[i].y, -(float)vec3d2[i].z);
        		f = -(float)vec3d2[i].x;
        		f1 = -(float)vec3d2[i].y;
        		f2 = -(float)vec3d2[i].z;
        	}
  ...
  //      		par1Tessellator.setColorRGBA_I(red<<16| green<<8| blue, alpha);
            ...
            renderer.pos((float)positiontexturevertex.vector3D.x * scale, 
            		(float)positiontexturevertex.vector3D.y * scale, 
            		(float)positiontexturevertex.vector3D.z * scale)
